...
import com.example.feesmanager.ui.announcements.AnnouncementsViewModel
...
class AnnouncementsActivity : BaseActivity() {
    ...
    private fun renderAnnouncements(list: List<Announcement>) {
        ...
        list.forEach { ann ->
            ...
            val time = runCatching { sdf.format(SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX", Locale.getDefault()).parse(ann.createdAt)!!) }.getOrElse { ann.createdAt.take(10) }
            card.addView(TextView(this).apply {
