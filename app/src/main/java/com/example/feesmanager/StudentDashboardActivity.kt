...
import com.example.feesmanager.ui.student.StudentDashboardViewModel
...
class StudentDashboardActivity : BaseActivity() {
        ...
        lifecycleScope.launch {
            try {
                ...
                when (enrollment?.status ?: "approved") {
                    "pending"  -> {
                        startActivity(Intent(this@StudentDashboardActivity,
    ...
    private fun loadUnreadBadges() {
        lifecycleScope.launch {
            val count = UnreadBadgeHelper.fetchTotalUnreadForStudent(currentTeacherId, currentStudentId)
            if (count > 0) {
                runCatching {
                    val bellBtn = findViewById<android.view.View>(R.id.btnAnnouncementsBell)
                    bellBtn?.let { UnreadBadgeHelper.addBadge(this@StudentDashboardActivity, it, count) }
                }
                runCatching {
                    val contactBtn = findViewById<android.view.View>(R.id.btnContactTeacher)
                    contactBtn?.let { UnreadBadgeHelper.addBadge(this@StudentDashboardActivity, it, count) }
                }
            }
        }
    }
    ...
    private fun setupButtons(studentId: String) {
        ...
        runCatching {
            ...
            val v = findViewById<View>(R.id.btnSwitchAccount)
                ?: return@runCatching
    ...
    @Serializable private data class EnrollmentStatusRow(val status: String)
    @Serializable private data class AcademyNameRow(val academy_name: String)
}
