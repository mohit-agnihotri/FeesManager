...
class DashboardActivity : BaseActivity() {
    ...
    private val viewModel: com.example.feesmanager.ui.dashboard.DashboardViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load unread badge on Class Chat button
        lifecycleScope.launch {
            val count = UnreadBadgeHelper.fetchTotalUnreadForTeacher(currentTeacherId)
            if (count > 0) {
                runCatching {
                    val chatBtn = findViewById<View>(R.id.btnStudentQueries)
                    UnreadBadgeHelper.addBadge(this@DashboardActivity, chatBtn, count)
                }
            }
        }
        ...
        viewModel.stats.observe(this) { result ->
            when (result) {
                is FmResult.Success -> {
                    ...
                    pending.text       = if (s.joinPending > 0)
                        "₹${s.totalPendingFees}  (+${s.joinPending} req)"
                    else
                        "₹${s.totalPendingFees}"
                }
    ...
    private fun setupButtons() {
        ...
        // Class chat (single button - duplicate removed)
        runCatching {
            findViewById<View>(R.id.btnStudentQueries).withBounce {
                startActivity(Intent(this, StudentQueriesActivity::class.java))
            }
        }
        ...
        runCatching {
            findViewById<View>(R.id.btnThemeToggle).withBounce {
                ThemeManager.toggle(this); recreate()
            }
        }
        runCatching {
            findViewById<View>(R.id.btnSwitchAccount).withBounce {
                lifecycleScope.launch {
                    SessionManager.logoutTeacher(this@DashboardActivity)
                    startActivity(Intent(this@DashboardActivity, RoleSelectActivity::class.java))
                    finish()
                }
            }
        }
        ...
        lifecycleScope.launch {
            try {
                val row = SupabaseManager.client.postgrest
                    .from("teachers")
                    .select(Columns.raw("vendor_status, kyc_status")) {
    ...
    private fun refreshUpiStatus(tvStatus: TextView?) {
        lifecycleScope.launch {
            try {
                val row = SupabaseManager.client.postgrest
                    .from("teachers")
                    .select(Columns.raw("razorpay_onboarding_status")) {
                        filter { eq("id", currentTeacherId) }
