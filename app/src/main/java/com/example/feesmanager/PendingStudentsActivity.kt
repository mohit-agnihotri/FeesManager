...
class PendingStudentsActivity : BaseActivity() {
    ...
    private fun buildRequestCard(sid: String, name: String, cls: String, joinedAt: String): LinearLayout {
        ...
        card.addView(TextView(this).apply { text = "👤 $name"; textSize = 16f; setTextColor(0xFFF1F5F9.toInt()); typeface = android.graphics.Typeface.DEFAULT_BOLD })
        card.addView(TextView(this).apply { text = "Class: $cls"; textSize = 13f; setTextColor(0xFF94A3B8.toInt()); val lp2 = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT); lp2.topMargin = 2; layoutParams = lp2 })
        if (joinedAt.isNotEmpty()) card.addView(TextView(this).apply { text = "Requested: $joinedAt"; textSize = 12f; setTextColor(0xFF64748B.toInt()); val lp2 = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT); lp2.topMargin = 2; layoutParams = lp2 })

        val btnRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.END; val lp2 = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT); lp2.topMargin = 12; layoutParams = lp2 }
        btnRow.addView(Button(this).apply {
            ...
            setOnClickListener { startActivity(Intent(this@PendingStudentsActivity, MessageActivity::class.java).putExtra("mode","teacher").putExtra("teacherId",teacherId).putExtra("studentId",sid).putExtra("studentName",name)) }
        })
        btnRow.addView(Button(this).apply {
    ...
    private fun setupSpinners(classList: List<String>) {
        ...
        spinnerClass.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p: AdapterView<*>?, v: View?, pos: Int, id: Long) { if (currentTab == "defaulters") renderCurrentTab() }
            override fun onNothingSelected(p: AdapterView<*>?) {}
        }
        val sorts = listOf("High to Low", "Low to High")
        spinnerSort.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, sorts)
    ...
    private fun renderDefaulters(selectedClass: String, sortType: String) {
        ...
        list.forEach { (name, cls, remain) ->
            ...
            card.addView(TextView(this).apply { text = "₹$remain"; textSize = 16f; setTextColor(0xFFEF4444.toInt()); typeface = android.graphics.Typeface.DEFAULT_BOLD })
