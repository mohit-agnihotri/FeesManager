        ...
        lifecycleScope.launch {
            try {
                val row = SupabaseManager.client.postgrest
                    .from("teachers")
                    .select(Columns.Companion.raw("razorpay_account_id, razorpay_onboarding_status")) {
    ...
    private fun connectBankAccount(name: String, number: String, ifsc: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Log.d("RAZORPAY_DEBUG", "SESSION = ${session != null}")
                Log.d("RAZORPAY_DEBUG", "TOKEN = ${session?.accessToken}")
                Log.d("RAZORPAY_DEBUG", "USER = ${session?.user?.id}")
