package com.example.feesmanager.ui.analytics
...
class AnalyticsActivity : BaseActivity() {
    ...
    private fun setupBarChart(monthlyData: TreeMap<String, Int>) {
        ...
        dataSet.colors = listOf(
            Color.parseColor("#6366F1"), Color.parseColor("#22C55E"),
    ...
    private fun setupPieChart(classData: HashMap<String, Int>) {
        ...
        classData.forEach { (cls, amount) -> if (amount > 0) entries.add(
            PieEntry(
                amount.toFloat(),
                "Class $cls"
            )
    ...
    private fun formatMonth(key: String) = try { SimpleDateFormat("MMMM yyyy", Locale.getDefault()).format(
        SimpleDateFormat("yyyy-MM", Locale.getDefault()).parse(key)!!) } catch (e: Exception) { key }
    private fun formatMonthShort(key: String) = try { SimpleDateFormat("MMM", Locale.getDefault()).format(
        SimpleDateFormat("yyyy-MM", Locale.getDefault()).parse(key)!!) } catch (e: Exception) { key }
}