...
class MultiAcademyActivity : BaseActivity() {
    ...
    private fun loadMyAcademies() {
        ...
        lifecycleScope.launch {
            try {
                ...
                enrollments.distinctBy { it.teacher_id }.forEach { enrollment ->
        ...
        try {
            ...
            if (teacher != null) {
                ...
                val enrollmentRow = SupabaseManager.client.postgrest.from("enrollments")
                    .select(Columns.Companion.raw("id, teacher_classes(class_name)")) {
                        filter {
                            eq("student_id", studentId)
    ...
    private fun buildCard(
        ...
        val tvAcademy = TextView(this).apply {
            ...
            textSize = 18f; setTextColor(0xFFF1F5F9.toInt()); setTypeface(null, Typeface.BOLD)
        }
