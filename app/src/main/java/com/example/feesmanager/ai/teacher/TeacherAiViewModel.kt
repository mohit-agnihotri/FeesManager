...
import com.example.feesmanager.ai.QuickAction
import com.example.feesmanager.ai.SuggestedAction
import com.example.feesmanager.ai.actions.ActionResult
import com.example.feesmanager.ai.actions.AiActionExecutor
...
class TeacherAiViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun showSmartWelcomeMessage(context: TeacherDataContext, issues: List<String>) {
        ...
        _messages.value = listOf(AiChatMessage(
            content = welcomeText,
            role = AiChatMessage.Role.ASSISTANT,
            priority = priority
        ))
