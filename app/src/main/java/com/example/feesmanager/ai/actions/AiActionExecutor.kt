    ...
    private suspend fun sendReminder(
        ...
        announcementsRepo.postAnnouncement(
            ...
            result = when (fmResult) {
                is com.example.feesmanager.data.FmResult.Success -> ActionResult(
                    success = true,
                    message = "✅ Reminder sent as announcement to all students!",
                    affectedCount = 1
                )
                is com.example.feesmanager.data.FmResult.Error -> ActionResult(
                    success = false,
                    message = "❌ Failed: ${fmResult.message}"
                )
                is com.example.feesmanager.data.FmResult.Loading -> ActionResult(
                    success = false,
                    message = "Processing..."
                )
            }
